...
class ForexDogV2(ForexDogBase):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        conditions = [
            ...
            qtpylib.crossed_above(df['close'], df['ema_3']),

            (df['ema_9'] - df['close']) / df['close'] > 0.015,
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Take profit when price touches the next slow EMA
        df.loc[
            (qtpylib.crossed_above(df['close'], df['ema_9'])),
            'exit_long'] = 1
...
class ForexDogV3(ForexDogBase):
    ...
    def custom_stoploss(self, pair: str, trade: 'Trade', current_time: 'datetime',
        ...
        return self.stoploss_from_absolute(stoploss_price, current_rate)
