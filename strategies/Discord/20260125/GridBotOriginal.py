...
# // GridV6_tmp7
# NOTE THAT THIS IS MARTINGALE AND HENCE UNFIT FOR TRADING
class GridBotOriginal(IStrategy):
    INTERFACE_VERSION = 2
    ...
    def calculate_state(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.dp.runmode.value in ("live", "dry_run"):
            ...
            if self.custom_info[pair][self.LIVE_DATE] == "":
                ...
                live_date_candle = dataframe.loc[
                    dataframe["date"] == self.custom_info[pair][self.LIVE_DATE]
                ]
        ...
        while row <= last_row:
            ...
            if (bot_state >= 1) and (bot_state <= self.max_dca_orders):
                ...
                if close <= grid_down:
                    ...
                    new_total_amount = (1 - pow(self.dca_scale, (bot_state + 1))) / (
                        1 - self.dca_scale
                    )
        ...
        # merge
        dataframe = pd.concat(
            [dataframe, df_buy_1, df_buy_2, df_sell_1, df_grid_up, df_grid_down, df_bot_state],
            axis=1,
        )
    ...
    def custom_stake_amount(
        ...
        **kwargs,
        ...
        if self.config["stake_amount"] == "unlimited":
            return (
                self.wallets.get_total_stake_amount() / self.max_dca_multiplier
            ) * self.stake_to_wallet_ratio
        else:
            ...
            if last_candle["buy_2"] == 1:
                ...
                if 0 < count_of_buys <= self.max_dca_orders:
                    try:
                        ...
                        stake_amount = stake_amount * pow(
                            self.dca_scale, (last_candle["bot_state"] - 1)
                        )
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        if not (self.dp.runmode.value in ("live", "dry_run")):
            dataframe = self.calculate_state(dataframe, metadata)
        dataframe.loc[:, "buy"] = 0
        dataframe.loc[(dataframe["buy_1"] == 1), "buy"] = 1
        return dataframe

    def populate_sell_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[:, "sell"] = 0
        dataframe.loc[(dataframe["sell_1"] == 1), "sell"] = 1
        return dataframe
