...
class ChannelBreakoutOvernightV2_Modified(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = "15m"
    can_short = True
    minimal_roi = {"0": 100}
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['high_max'] = dataframe['high'].rolling(window=50).max()
        dataframe['low_min'] = dataframe['low'].rolling(window=50).min()
        return dataframe

    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        is_weekend = (dataframe["date"].dt.dayofweek >= 5) if self.disable_weekends.value else pd.Series([False] * len(dataframe))
        long_cond = (dataframe["close"] > dataframe['high_max'].shift(1))
        short_cond = (dataframe["close"] < dataframe['low_min'].shift(1))
