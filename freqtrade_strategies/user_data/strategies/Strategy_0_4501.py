...
class Strategy_0_4501(IStrategy):
    ...
    indicator_ma_period_1 = IntParameter(50, 85, default=68, space="buy")
    indicator_crs_ma_period = IntParameter(20, 40, default=30, space="buy")
    mt_atr_period = IntParameter(15, 25, default=20, space="buy")
    bb_bar_opens_period = IntParameter(25, 50, default=37, space="buy")
    wpr_cross_period = IntParameter(15, 30, default=20, space="buy")
    indicator_ma_period_2 = IntParameter(60, 95, default=79, space="buy")
    bb_bar_closes_period = IntParameter(35, 65, default=50, space="buy")
    ma_period = IntParameter(35, 65, default=50, space="sell")
    bs_power_period = IntParameter(10, 20, default=15, space="sell")

    # Entry/Exit parameters
    price_entry_mult = DecimalParameter(0.1, 0.5, default=0.3, space="buy")
    exit_after_bars = IntParameter(3, 8, default=5, space="sell")
    profit_target_coef = DecimalParameter(3.5, 6.0, default=4.7, space="sell")
    stop_loss_pct = DecimalParameter(1.0, 3.0, default=2.0, space="sell")
    keltner_channel_period = IntParameter(15, 30, default=20, space="buy")
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["session_high"] = dataframe["session_high"].fillna(method="ffill")
        dataframe["session_low"] = dataframe["session_low"].fillna(method="ffill")

        # Session Close (7:38)
        dataframe["session_close"] = dataframe.apply(
            lambda row: row["close"]
            if (row["hour"] == 7 and 30 <= row["minute"] < 45)
            else np.nan,
            axis=1,
        ).fillna(method="ffill")
        ...
        dataframe["kc_upper"] = kc[f"KCUp_{self.keltner_channel_period.value}_2.0"]
        dataframe["kc_lower"] = kc[f"KCLe_{self.keltner_channel_period.value}_2.0"]
        ...
        informative_1h["high_weekly_1h"] = (
            informative_1h["high"].rolling(window=7 * 24).max()
        )
        informative_1h["low_weekly_1h"] = (
            informative_1h["low"].rolling(window=7 * 24).min()
        )
        ...
        informative_1h["session_high_1h"] = informative_1h["session_high_1h"].fillna(
            method="ffill"
        )
        informative_1h["session_low_1h"] = informative_1h["session_low_1h"].fillna(
            method="ffill"
        )
        ...
        informative_1h["bb_upper_1h"] = bb_1h["upperband"]
        informative_1h["bb_lower_1h"] = bb_1h["lowerband"]
    ...
    def fuzzy_logic_long_entry(self, dataframe: DataFrame, index: int) -> bool:
        ...
        if index > 2:
            conditions.append(
                dataframe.iloc[index - 1]["ha_close"]
                < dataframe.iloc[index - 2]["high_monthly"]
            )
        else:
        ...
        if index > 3:
            conditions.append(
                (
                    dataframe.iloc[index - 4]["mtatr"]
                    > dataframe.iloc[index - 4]["mtatr_ema"]
                )
                and (
                    dataframe.iloc[index - 3]["mtatr"]
                    < dataframe.iloc[index - 3]["mtatr_ema"]
                )
            )
        else:
            conditions.append(False)
        ...
        if index > 2:
            conditions.append(
                dataframe.iloc[index - 1]["session_low_1h"]
                < dataframe.iloc[index - 2]["low_weekly_1h"]
            )
        else:
            conditions.append(False)
        ...
        if index > 1:
            conditions.append(
                ...
                < dataframe.iloc[index - 1]["bb_lower_1h"]
            )
        else:
            conditions.append(False)
    ...
    def fuzzy_logic_short_entry(self, dataframe: DataFrame, index: int) -> bool:
        ...
        if index > 2:
            conditions.append(
                dataframe.iloc[index - 1]["ha_close"]
                > dataframe.iloc[index - 2]["low_monthly"]
            )
        else:
            conditions.append(False)
        ...
        if index > 2:
            conditions.append(
                dataframe.iloc[index - 1]["session_high_1h"]
                > dataframe.iloc[index - 2]["high_weekly_1h"]
            )
        else:
            conditions.append(False)
        ...
        if index > 1:
            conditions.append(
                dataframe.iloc[index - 1]["close"]
                > dataframe.iloc[index - 1]["bb_upper_1h"]
            )
        else:
            conditions.append(False)
    ...
    def fuzzy_logic_long_exit(self, dataframe: DataFrame, index: int) -> bool:
        ...
        if index > 2:
            conditions.append(
                dataframe.iloc[index - 2]["smma"] > dataframe.iloc[index - 3]["smma"]
            )
        else:
            conditions.append(False)
        ...
        if index > 8:
            ...
            for i in range(5):
                bar_index = index - 3 - i
                if (
                    dataframe.iloc[bar_index]["ha_close_1h"]
                    >= dataframe.iloc[bar_index - 1]["ha_close_1h"]
                ):
                    condition_met = False
                    break
            conditions.append(condition_met)
