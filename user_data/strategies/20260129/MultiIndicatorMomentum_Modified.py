...
class MultiIndicatorMomentum_Modified(IStrategy):
    """
    Modified MultiIndicatorMomentum:
    ...
    # Exit thresholds (Trailing stop params)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        """
        # Weekend filter
        if self._get_param_value(self.disable_weekends):
            # 5 = Saturday, 6 = Sunday
        ...
        # Apply conditions
        if conditions_long:
            dataframe.loc[reduce(lambda x, y: x & y, conditions_long) & (~is_weekend), "enter_long"] = 1

        if conditions_short:
            dataframe.loc[reduce(lambda x, y: x & y, conditions_short) & (~is_weekend), "enter_short"] = 1
