...
class WednesdayStrategy(IStrategy):
    ...
    - Use ATR as volatility/impulse context (ATR% vs baseline), not as an impossible one-candle jump.
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['hma'] = qtpylib.hma(dataframe, period=int(self.buy_hma_period.value))
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        wed = dataframe['day_of_week'] == self.buy_day_of_week.value
        ...
        # --- LONG ---
        # Wednesday + time + liquidity + (ATR context OR MA cross) + UO low + AO supportive + regime ok
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            qtpylib.crossed_below(dataframe['zlema'], dataframe['hma']) | (dataframe['ao'] < -ao_gate),
            'exit_long'
        ] = 1

        dataframe.loc[
            qtpylib.crossed_above(dataframe['zlema'], dataframe['hma']) | (dataframe['ao'] > ao_gate),
            'exit_short'
        ] = 1
